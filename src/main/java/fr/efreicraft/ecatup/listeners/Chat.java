...
public class Chat implements Listener {
    ...
    public void onChat(AsyncChatEvent event) {
        ...
        Component msg = Component.text(channelActuel == PreferenceCache.ChatChannel.SERVER ? "" : ("["+channelActuel+"]")).color(NamedTextColor.YELLOW).append(event.getPlayer().displayName())
        ...
        msgCopie.insert(0, "["+channelActuel+"] ");

        switch (channelActuel) {
            case SERVER -> Bukkit.broadcast(msg
            ...
            case TEAM -> {
                if (event.getPlayer().getScoreboardTags().isEmpty()) {
                    ...
                    for (String playerName : team.getEntries()) {
                        if (Bukkit.getPlayer(playerName) != null) Bukkit.getPlayer(playerName).sendMessage(msg
                                .hoverEvent(HoverEvent.hoverEvent(HoverEvent.Action.SHOW_TEXT, Component.text("Copier")))
                                .clickEvent(ClickEvent.copyToClipboard(msgCopie + ": " + ((TextComponent) event.message()).content() + WHAT_TIME_IS_IT)));
                    }
                }
